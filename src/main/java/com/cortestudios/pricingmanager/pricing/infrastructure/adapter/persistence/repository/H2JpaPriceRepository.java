...
public interface H2JpaPriceRepository extends JpaRepository<PriceEntity, Long> {
    ...
    @Query(value = "SELECT p.* FROM prices p WHERE p.brand_id = :brandId AND p.product_id = :productId AND :applicationDate BETWEEN p.start_date AND p.end_date ORDER BY p.priority DESC LIMIT 1",
             nativeQuery = true)
    ...
    @Query(value = "SELECT p.* FROM prices p WHERE p.brand_id = :brandId AND p.product_id = :productId ORDER BY p.priority DESC",
            nativeQuery = true)
